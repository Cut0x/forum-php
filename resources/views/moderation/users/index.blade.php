<x-moderation-layout title="Utilisateurs" active="users">
    ...
    <div class="space-y-3">
        @foreach($users as $user)
            <div x-data="{ warn: false, suspend: false }" class="card p-4">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    @if(auth()->user()->can('moderate', $user))
                        <div class="flex gap-2">
                            ...
                            @if($user->isSuspended())
                                <form method="post" action="{{ route('moderation.users.unsuspend', $user) }}">
                                    @csrf
                ...
                <form x-show="warn" x-cloak method="post" action="{{ route('moderation.users.warn', $user) }}" class="mt-3 flex gap-2">
                    @csrf
                ...
                <form x-show="suspend" x-cloak method="post" action="{{ route('moderation.users.suspend', $user) }}" class="mt-3 flex gap-2">
                    @csrf
