<x-admin-layout title="Footer" active="footer">
    <div class="grid gap-4 sm:grid-cols-2">
        <form method="post" action="{{ route('admin.footer.categories.store') }}" class="card space-y-2 p-4">
            @csrf
        ...
        <form method="post" action="{{ route('admin.footer.links.store') }}" class="card space-y-2 p-4">
            @csrf
        ...
        @foreach($categories as $category)
            <div class="card p-4">
                <div class="flex items-center justify-between">
                    ...
                    <form method="post" action="{{ route('admin.footer.categories.destroy', $category) }}" onsubmit="return confirm('Supprimer cette catégorie et ses liens ?');">
                        @csrf @method('delete')
                        <button type="submit" class="text-sm text-muted hover:text-red-600">Supprimer</button>
                    </form>
                </div>
