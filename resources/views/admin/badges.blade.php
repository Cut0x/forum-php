<x-admin-layout title="Badges" active="badges">
    <form method="post" action="{{ route('admin.badges.store') }}" class="card space-y-3 p-5">
        @csrf
    ...
    <div class="grid gap-3 sm:grid-cols-2">
        @foreach($badges as $badge)
            <form method="post" action="{{ route('admin.badges.update', $badge) }}" class="card space-y-2 p-4">
                @csrf @method('patch')
