<x-admin-layout title="Catégories" active="categories">
    <form method="post" action="{{ route('admin.categories.store') }}" class="card space-y-3 p-5">
        @csrf
    ...
    <div class="space-y-3">
        @foreach($categories as $category)
            <div x-data="{ editing: false }" class="card p-4">
                ...
                <form x-show="editing" x-cloak method="post" action="{{ route('admin.categories.update', $category) }}" class="space-y-3">
                    @csrf @method('patch')
                    <div class="grid gap-3 sm:grid-cols-2">
